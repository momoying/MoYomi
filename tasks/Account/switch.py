...
from tasks.Account.assets import SwitchAssets

ASSETS = SwitchAssets
TEMPLATES = ASSETS.TEMPLATES
AVATAR_CLICK_REGION = ASSETS.AVATAR_CLICK_REGION
REGIONS = ASSETS.REGIONS
LOGIN_ACCOUNT_REGION = ASSETS.LOGIN_ACCOUNT_REGION
LOGIN_BUTTON_REGION = ASSETS.LOGIN_BUTTON_REGION
ACCOUNT_LIST_REGION = ASSETS.ACCOUNT_LIST_REGION
COUNT_FLAG_SEARCH_REGION = ASSETS.COUNT_FLAG_SEARCH_REGION
ACCOUNT_TEXT_RIGHT = ASSETS.ACCOUNT_TEXT_RIGHT
MATCH_THRESHOLD = ASSETS.MATCH_THRESHOLD
COUNT_FLAG_THRESHOLD = ASSETS.COUNT_FLAG_THRESHOLD
ACCOUNT_LIST_MAX_SWIPES = ASSETS.ACCOUNT_LIST_MAX_SWIPES
ACCOUNT_LIST_SWIPE_DURATION_MS = ASSETS.ACCOUNT_LIST_SWIPE_DURATION_MS
...
ACCOUNT_STATUS_PATH = PROJECT_ROOT / "config" / "account_status.json"
...
def _save_timeout_screenshot(stage: str, frame=None) -> Optional[Path]:
    saved_path = capture_error_screenshot("account_switch", stage, frame=frame)
    if saved_path is not None:
        print(f"账号切换报错截图已保存: {saved_path}")
...
def scan_account_list(
    ...
    for page_index in range(ACCOUNT_LIST_MAX_SWIPES + 1):
        ...
        DEVICE.swipe(840, 548, 840, 338, duration_ms=ACCOUNT_LIST_SWIPE_DURATION_MS)
