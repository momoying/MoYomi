...
界面和模板均以 1280x720 分辨率制作。截图由 MoYomi/Core 中的
``game_automation.py`` 提供，并统一限制为每 0.5 秒一帧。
"""
...
# game_automation.take_screenshot() 会根据这个配置自动限速。
...
TEMPLATES = {
    ...
    "flag": str(SCRIPT_DIR / "flag.png"),
    "select": str(SCRIPT_DIR / "select.png"),
    "join": str(SCRIPT_DIR / "join.png"),
    "refresh": str(SCRIPT_DIR / "reflash.png"),
    "prepare": str(SCRIPT_DIR / "prepare.png"),
    "battle": str(SCRIPT_DIR / "battle.png"),
    "win": str(SCRIPT_DIR / "win.png"),
    "defeat": str(SCRIPT_DIR / "defeat.png"),
    "back": str(SCRIPT_DIR / "back.png"),
    "exit_confirm": str(SCRIPT_DIR / "exit_confirm.png"),
    "phone_bind": str(SCRIPT_DIR / "phone_bind.png"),
    "phone_bind_cancel": str(SCRIPT_DIR / "phone_bind_cancel.png"),
}

# 识别范围来自项目中的 1280x720 样图，后续如 UI 发生变化只需修改这里。
REGIONS = {
    "main": ((500, 80), (800, 280)),
    # 底部菜单会因入口数量变化而横向移动，范围覆盖组队按钮的常见位置。
    "team": ((300, 560), (600, 710)),
    "menu_scroll": ((1100, 540), (1280, 720)),
    "flag": ((390, 105), (610, 195)),
    # 同心队会继承组队页最后停留的副本分类，经验妖怪可能不再位于
    # 原先的固定行，因此在整条左侧副本栏内寻找。
    "select": ((120, 100), (380, 700)),
    "join": ((980, 160), (1200, 300)),
    "refresh": ((390, 570), (650, 690)),
    "prepare": ((1050, 490), (1280, 690)),
    "battle": ((0, 0), (360, 100)),
    "win": ((250, 0), (950, 210)),
    "defeat": ((250, 0), (950, 330)),
    "back": ((0, 0), (120, 100)),
    "exit_confirm": ((600, 350), (920, 520)),
    "phone_bind": ((880, 390), (1160, 640)),
    "phone_bind_cancel": ((380, 400), (680, 610)),
}
...
def _select_exp_monster_if_needed() -> bool:
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            utils.adb_swipe(x, start_y, x, end_y, duration_ms)
